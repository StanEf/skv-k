        ...
        while ($object = $ObjectList->fetch()) {
           /* echo '$object<pre>';
            ...
            $actions = Array(
                ...
                array("ICONCLASS" => "delete", "TEXT" => Loc::getMessage('SKV_LC_CLIENTS_LIST_DELETE_BUTTON'), "ONCLICK" => "if(confirm('" . Loc::getMessage('SKV_LC_CLIENTS_LIST_DELETE_CONFIRM') . "')) window.location='?action=delete_client&ID=" . $object["ID"] . "&" . bitrix_sessid_get() . "';"),
            );
            $cols = array();
            $res = array();
