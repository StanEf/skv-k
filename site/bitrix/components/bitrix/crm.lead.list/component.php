...
if (!empty($sExportType) && $userPermissions->HavePerm('LEAD', BX_CRM_PERM_NONE, 'EXPORT'))
{
	ShowError(GetMessage('CRM_PERMISSION_DENIED'));
	return;
}
...
if (!$bInternal)
{
	...
	$arResult['FILTER'] = array(
		...
		array(
			...
			'name' => GetMessage('CRM_COLUMN_COMMUNICATION_TYPE'),
	...
	$arResult['FILTER'][] = Bitrix\Crm\PhaseSemantics::getListFilterInfo(
		CCrmOwnerType::Lead,
		array(
			'id' => 'STATUS_SEMANTIC_ID',
			'name' => GetMessage('CRM_COLUMN_STATUS_SEMANTIC_ID'),
			'default' => true,
			'params' => array('multiple' => 'Y')
		),
		true
	...
	$arResult['FILTER'][] = \Bitrix\Crm\Counter\EntityCounterType::getListFilterInfo(
		array(
			'id' => 'ACTIVITY_COUNTER',
			'name' => GetMessage('CRM_COLUMN_ACTIVITY_COUNTER'),
			'default' => true
		)
	);
...
if ($sExportType == '')
{
	$schemeID = \Bitrix\Crm\Conversion\LeadConversionConfig::getCurrentSchemeID();
