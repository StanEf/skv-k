...
class CCrmWebFormEditComponent extends \CBitrixComponent
{
	...
	public function processPost()
	{
		...
		$useLicence = $request->get('USE_LICENCE') == 'Y';
		...
		$params = array(
			...
			'LICENCE_BUTTON_CAPTION' => $useLicence ? $request->get('LICENCE_BUTTON_CAPTION') : '',
			'LICENCE_TEXT' => $useLicence ? $request->get('LICENCE_TEXT') : '',
		...
		if(!$this->crmWebForm->hasErrors())
		{
			...
			$this->redirectTo(!$this->request->get('submit_apply'));
		}
		else
		{
	...
	protected function redirectTo($toList = true)
	{
		$url = $toList ? $this->arParams['PATH_TO_WEB_FORM_LIST'] : $this->arParams['PATH_TO_WEB_FORM_EDIT'];
	...
	protected function setResultCallBackForm()
	{
		...
		$callFromList = CVoxImplantConfig::GetPortalNumbers(true);
		foreach($callFromList as $phoneCode => $phoneCaption)
		{
			$this->arResult['CALL_BACK_FORM']['CALL_FROM'][] = array(
				'ID' => $phoneCode,
				'VALUE' => $phoneCaption,
				'SELECTED' => $phoneCode == $this->arResult['FORM']['CALL_FROM'],
			);
		}
	}
	...
	public function executeComponent()
	{
		...
		global $APPLICATION;
		$APPLICATION->SetTitle(intval($this->arParams['ELEMENT_ID']) > 0 ? Loc::getMessage('CRM_WEBFORM_EDIT_TITLE') : Loc::getMessage('CRM_WEBFORM_EDIT_TITLE_ADD'));
		$this->prepareResult();
