var CrmFormEditor = function(params)
{
	...
	this.initInterface = function()
	{
		...
		this.bindEditInline(BX('LICENCE_CONTAINER'), 'dynamic-field');

		//checkboxes
		BX.bind(BX('USE_LICENCE'), 'click', function(){
			BX.toggleClass(BX('LICENCE_CONTAINER'), 'crm-webform-display-none');
		});
			...
			BX.bind(copyButton, 'click', BX.proxy(function(e){
				...
				BX.ajax({
					...
					onsuccess: BX.proxy(function(data){
						...
						{
							window.location = this.detailPageUrlTemplate.replace('#id#', data.copiedId).replace('#form_id#', data.copiedId);
						}
					}, this),
...
CrmWebFormEditEntityScheme.prototype =
{
	init: function ()
	{
		...
		this.submitButtonNode = BX('CRM_WEBFORM_SUBMIT_BUTTON');
		BX.bind(this.submitButtonNode, 'click', BX.proxy(this.checkCreateFields, this));
		BX.bind(BX('CRM_WEBFORM_SUBMIT_APPLY'), 'click', BX.proxy(this.checkCreateFields, this));
			...
			this.popupConfirmCreateFields.setButtons([
				...
				new BX.PopupWindowButton({
					...
					events: {
						click: BX.proxy(function(){
							this.submitButtonEnabled = true;
							BX.removeClass(this.submitButtonNode, 'crm-webform-submit-button-loader');
	...
	actualizeTextWillCreatedEntities: function()
	{
		...
		this.getCurrentWillCreatedEntities().forEach(function(entityTypeName){
			entityTypeCaptions.push(this.caller.entityDictionary[entityTypeName].toLowerCase());
		}, this);
