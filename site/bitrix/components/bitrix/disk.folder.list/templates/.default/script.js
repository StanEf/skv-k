...
BX.Disk.FolderListClass = (function (){
	...
	FolderListClass.prototype.showInfoPanel = function(params)
	{
		...
		if(!isFolder && objectId && !isMany && this.enabledExternalLink)
		{
								...
								'<li>' +
									...
									'<table id="bx-disk-ext-use-password-table" class="bx-disk-popup-share-password" ' + (response.linkData.hasPassword? '' : 'style="display: none;"') + '>' +
										'<tbody><tr>' +
											...
											'<td><input id="bx-disk-ext-use-password-value" type="password" readonly onfocus="this.removeAttribute(\'readonly\');" class="bx-disk-popup-input" style="height: 31px;"></td>' +
										'</tr>' +
									'</tbody></table>' +
								'</li>' +
							'</ul>' +
	...
	{
		BX.Disk.modalWindowLoader(BX.Disk.addToLinkParam(this.ajaxUrl, 'action', 'getDetailSettingsExternalLinkForceCreate'), {
			id: 'bx-disk-external-link-loader',
			responseType: 'json',
			postData: {
				objectId: objectId
							...
							'<ul class="bx-disk-popup-share-link-param-list">' +
								...
								'<li>' +
									...
									'<table id="bx-disk-ext-use-password-table" class="bx-disk-popup-share-password" ' + (response.linkData.hasPassword? '' : 'style="display: none;"') + '>' +
										'<tbody><tr>' +
											'<td>' + BX.message('DISK_FOLDER_LIST_TITLE_EXT_PARAMS_INPUT_PASSWORD') + ':</td>' +
											'<td><input autocomplete="false" id="bx-disk-ext-use-password-value" type="password" readonly onfocus="this.removeAttribute(\'readonly\');" class="bx-disk-popup-input" style="height: 31px;"></td>' +
										'</tr>' +
									'</tbody></table>' +
								'</li>' +
							'</ul>' +
