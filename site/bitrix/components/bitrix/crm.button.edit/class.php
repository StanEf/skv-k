...
class CCrmSiteButtonEditComponent extends \CBitrixComponent
{
	...
	protected function prepareLanguages()
	{
		...
		global $b24Languages;
		$fileName = Bitrix\Main\Application::getDocumentRoot() . SITE_TEMPLATE_PATH . "/languages.php";
		include_once $fileName;
		if (isset($b24Languages) && is_array($b24Languages))
		{
			$this->arResult['LANGUAGES']['LIST'] = \Bitrix\Main\Text\Encoding::convertEncoding($b24Languages, 'UTF-8', SITE_CHARSET);
			$this->arResult['LANGUAGES']['CURRENT'] = $this->button->getLanguageId();
		}
	}
	...
	protected function prepareResultItems()
	{
		...
		foreach (Manager::getWidgetList() as $typeItem)
		{
			...
			{
				foreach ($typeItem['RESOURCES'] as $resource)
				{
					if ($resource['type'] == 'text/css')
					{
						$this->arResult['ADDITIONAL_CSS'][] = $resource['path'];
					}
				}
			}
