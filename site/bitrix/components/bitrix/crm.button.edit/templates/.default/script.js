var CrmButtonEditor = function(params) {
	...
	{
		var langs = JSON.parse(this.languageContext.getAttribute("data-langs"));
		var items = [];
		var _this = this;
		for (var lang in langs)
		{
			(function(lang) {
				items.push({
					...
					{
						this.close();
						_this.changeLanguage(lang, langs[lang]);
					}
				});
			})(lang);
		}
	...
	onChangeSelect: function(node)
	{
		...
		if (type == 'openline')
		{
			...
			connectors.forEach(function (item) {
				...
				BX.addClass(node, 'crm-button-edit-channel-lines-social-items');
				BX.addClass(node, 'connector-icon');
				BX.addClass(node, 'connector-icon-' + item.code.replace('.', '-'));
