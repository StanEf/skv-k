...
if ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	__CrmCompanyListEndResonse($arData);
}
elseif ($action === 'REBUILD_SEARCH_CONTENT')
{
	...
	if(!$agent->isEnabled())
	{
		__CrmCompanyListEndResonse(array('STATUS' => 'COMPLETED'));
	}
...
elseif ($action === 'GET_ROW_COUNT')
{
	...
	{
		__CrmCompanyListEndResonse(array('DATA' => array('TEXT' => '')));
	}
	...
	__CrmCompanyListEndResonse(array('DATA' => array('TEXT' => $text)));
}
...
elseif ($action === 'BUILD_REQUISITES')
{
	...
	if($presetID <= 0)
	{
		__CrmCompanyListEndResonse(array('ERROR' => "Preset ID is not specified."));
	}
...
elseif ($action === 'CONVERT_REQUISITES')
{
	...
	if($itemQty > 0)
	{
		foreach($converters as $converter)
		{
			...
			{
				__CrmCompanyListEndResonse(array('ERROR' => $e->getLocalizedMessage()));
			}
			catch(Exception $e)
			{
				__CrmCompanyListEndResonse(array('ERROR' => $e->getMessage()));
			}
		}
