...
elseif ($bEdit || $bCopy)
{
	...
	if ($arParams['IS_RECURRING'] === "Y")
	{
		...
		{
			$result = Bitrix\Crm\Recurring\Manager::exposeInvoices(
				array(
...
if($bProcessPost && isset($arResult['EXTERNAL_CONTEXT']) && $arResult['EXTERNAL_CONTEXT'] !== '')
{
...
else
{
	...
	{
		$bVarsFromForm = true;
		if (isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']) || $bAjaxSubmit)
		{
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				if ($bEdit)
				{
					...
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y')
					{
						...
						if ($_POST['RECUR_PARAM']['REPEAT_TILL'] === 'times' && (int)$recurringList['LIMIT_REPEAT'] > 0)
						{
							$recurringList['IS_LIMIT'] = 'T';
						}
						elseif($_POST['RECUR_PARAM']['REPEAT_TILL'] === 'date')
						{
						...
						$res = \Bitrix\Crm\Recurring\Manager::updateRecurring($recur['ID'], $recurringList);

						if ($res->isSuccess())
						{
							\Bitrix\Crm\Recurring\Manager::exposeTodayInvoices();
						}
					}
				}
				else
				{
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y')
					{
						...
						$result = \Bitrix\Crm\Recurring\Manager::createInvoice($arFields, $recurringList);
...
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'UF_MYCOMPANY_ID',
	'name' => GetMessage('CRM_INVOICE_FIELD_UF_MYCOMPANY_ID'),
