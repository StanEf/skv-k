...
class CrmActivityVisitComponent extends \CBitrixComponent
{
	...
	public static function recognizeFace($fields, $userId)
	{
		...
		$recognizeResponse = \Bitrix\FaceId\FaceId::identify($rawImage);
	...
	protected function prepareDataForEdit()
	{
		...
		$result['FACEID_ENABLED'] = Main\ModuleManager::isModuleInstalled('faceid') && $this->arParams['HAS_RECOGNIZE_CONSENT'];
		return $result;
	}
	...
	protected static function updateRecognizePicture($entityTypeId, $entityId)
	{
		...
		$rawImage = $imageFile->getContents();
		$recognizeResponse = \Bitrix\FaceId\FaceId::add($rawImage);
