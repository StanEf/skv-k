...
if (!$bInternal)
{
	$arResult['FILTER'] = array(
		...
		array(
			'id' => 'UF_MYCOMPANY_ID',
			'name' => GetMessage('CRM_COLUMN_UF_MYCOMPANY_ID'),
			'type' => 'custom_entity',
			'selector' => array(
				'TYPE' => 'crm_entity',
				'DATA' => array(
...
if ($arParams['IS_RECURRING'])
{
	$arResult['HEADERS'] = array(
		...
		array('id' => 'UF_MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_UF_MYCOMPANY_ID'), 'editable' => false),
...
else
{
	$arResult['HEADERS'] = array(
		...
		array('id' => 'UF_MYCOMPANY_ID', 'name' => GetMessage('CRM_COLUMN_UF_MYCOMPANY_ID'), 'default' => false, 'editable' => false),
...
else
{
	...
	if (key($arSort))
	{
		...
		{
			$sort = array($recurFieldPrefix . strtoupper(key($arSort)) => current($arSort));
		}
		elseif ($inHeader)
		{
	...
	foreach (array_keys($arFilter) as $filterField)
	{
		...
		{
			$filter[str_replace('~','',$filterField)] = $arFilter[$filterField];
		}
	}
