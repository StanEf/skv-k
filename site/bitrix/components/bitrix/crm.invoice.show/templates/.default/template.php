	...
	BX.ajax({
		...
		onsuccess: BX.delegate(function(result) {
			...
			if(result)
			{
				if(!result.ERROR)
				{
					BX.PopupMenu.show('externale-link-<?=CUtil::JSEscape($arResult['ELEMENT_ID'])?>', event, [{text : '<input type="text" value="'+result.LINK+'" style="margin-top: 6px; width : 100%" id="generated-link"><span class="crm-invoice-edit-url-link-icon" title="<?=GetMessage('CRM_INVOICE_PUBLIC_LINK_COPY');?>" id="clipboard-copy"></span>'}], {angle : {offset : 80, position : 'top'}});
