...
//						li.style.opacity = state.opacity / 100;
	...
	setEstimatedTime : function(percent)
	{
		var nowTime = (new Date()).getTime() / 1000;
		var startTime = this.bxim.getLocalConfig("startPackageTime", null);
		if(startTime && percent > 0)
		{
			var estimatedTime = this.getTextForEstimatedTime(((nowTime - startTime/1000 ) / percent) * (100 - percent));
	...
	prepareDataToHistoryItem : function(data)
	{
		...
		else if(data.snapshot.id && data.isDownloaded)
		{
			fileData.isNew = true;
		}
