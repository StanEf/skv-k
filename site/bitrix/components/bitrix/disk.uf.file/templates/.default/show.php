...
?><div id="disk-attach-block-<?=$arResult['UID']?>" class="feed-com-files diskuf-files-entity"><?
	...
	if (!empty($arResult['FILES']))
	{
		...
		{
			?><div class="<?=$className?>">
				<span id="lock-anchor-created-<?= $file['ID'] ?>-<?= $component->getComponentId() ?>" class="feed-con-file-icon feed-file-icon-<?=htmlspecialcharsbx($file['EXTENSION'])?> js-disk-locked-document-tooltip">
				<? if($file['IS_LOCKED']) { ?>
				...
				<span class="feed-com-file-name-wrap">
					...
					?><script type="text/javascript">
						...
						BX.Disk.Files['<?= $file['ID'] ?>'] = [
							...
							{text : BX.message('JS_CORE_VIEWER_EDIT'), className : "bx-viewer-popup-item item-edit", href : "#", onclick: function(e){
								BX.fireEvent(BX.findPreviousSibling(BX(this.bindElement), function(node){ return BX.type.isElementNode(node) && (node.getAttribute('data-bx-viewer'));}, true), 'click'); (!BX.CViewer.isLocalEditService(BX.message('disk_document_service')) && top.BX.CViewer.objNowInShow.runActionByCurrentElement('forceEdit', {obElementViewer: top.BX.CViewer.objNowInShow}));
								BX.PopupMenu.currentItem.popupWindow.close();
								return BX.PreventDefault(e);
							}},
							<? } ?>
					...
					<? if($file['IS_LOCKED']) { ?>
						BX.tooltip(
							<?php echo $file['CREATED_BY']?>,
							"lock-anchor-created-<?= $file['ID'] ?>-<?= $component->getComponentId() ?>"
						);
					<? } ?>

					</script><?
					if($file['EDITABLE'] && $file['CAN_UPDATE'] && (!$file['IS_LOCKED'] || $file['IS_LOCKED_BY_SELF'])) {
						?><a class="feed-con-file-changes-link" href="#" onclick="BX.fireEvent(BX.findPreviousSibling(BX(this), function(node){ return BX.type.isElementNode(node) && (node.getAttribute('data-bx-viewer'));}, true), 'click');(!BX.CViewer.isLocalEditService(BX.message('disk_document_service')) && top.BX.CViewer.objNowInShow.runActionByCurrentElement('forceEdit', {obElementViewer: top.BX.CViewer.objNowInShow})); return false;"><?= GetMessage('WDUF_FILE_EDIT') ?></a><?
					}
	...
	if(
		...
		(!empty($arResult['IMAGES']) || !empty($arResult['FILES'])) &&
		(count($arResult['IMAGES']) + count($arResult['FILES']) > 1)
	)
	{
		?>
